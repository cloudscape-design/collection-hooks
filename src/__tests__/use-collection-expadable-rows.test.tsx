...
test('displays root items and expanded items children only', () => {
  const { visibleItems, result } = renderUseCollection(items, {
    expandableRows: { getId, getParentId, defaultExpandedItems: [items[4]] },
  });
  expect(visibleItems.map(i => i.id)).toEqual(['a', 'b', 'b.1', 'c']);
  expect(result.collectionProps.totalItemsCount).toBe(3); // root items only
});
...
test('displays root items and expanded items children only in a deep tree', () => {
  ...
  expect(result.collectionProps.totalItemsCount).toEqual(3); // root items only
});

test('total count equals the number of root nodes', () => {
  for (let totalItems = 0; totalItems <= 25; totalItems += 1) {
    const items = generateRandomNestedItems({ totalItems });
    const rootItems = items.filter(i => !i.id.includes('.'));
    const { result } = renderUseCollection(items, { expandableRows: { getId, getParentId } });
    expect(result.collectionProps.totalItemsCount).toBe(rootItems.length);
  }
});
